...
DiscreteMeshletLoD::DiscreteMeshletLoD(const std::wstring& name, int width, int height, bool vSync)
...
void DiscreteMeshletLoD::UpdateBufferResource(
...
void DiscreteMeshletLoD::CreatePSO()
{
...
void DiscreteMeshletLoD::CreateCullingPSO()
{
...
bool DiscreteMeshletLoD::LoadContent()
{
    auto device = Application::Get().GetDevice();
...
void DiscreteMeshletLoD::ResizeDepthBuffer(int width, int height)
{
    if (m_ContentLoaded)
    {
...
void DiscreteMeshletLoD::OnResize(ResizeEventArgs& e)
{
...
void DiscreteMeshletLoD::UnloadContent()
{
    m_ContentLoaded = false;
...
void DiscreteMeshletLoD::OnUpdate(UpdateEventArgs& e)
{
...
void DiscreteMeshletLoD::UpdateObjectsBuffer()
{
    Application::Get().Flush();
    auto device = Application::Get().GetDevice();
    auto commandQueue = Application::Get().GetCommandQueue(D3D12_COMMAND_LIST_TYPE_COPY);
...
void DiscreteMeshletLoD::TransitionResource(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList7> commandList,
...
void DiscreteMeshletLoD::ClearRTV(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList7> commandList,
    D3D12_CPU_DESCRIPTOR_HANDLE rtv, FLOAT* clearColor)
{
    commandList->ClearRenderTargetView(rtv, clearColor, 0, nullptr);
}

void DiscreteMeshletLoD::ClearDepth(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList7> commandList,
    D3D12_CPU_DESCRIPTOR_HANDLE dsv, FLOAT depth)
{
    commandList->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH, depth, 0, 0, nullptr);
}

void DiscreteMeshletLoD::OnRender(RenderEventArgs& e)
{
    super::OnRender(e);
...
void DiscreteMeshletLoD::OnKeyPressed(KeyEventArgs& e)
{
...
void DiscreteMeshletLoD::updateImGui()
{
    ...
    {
        if (ImGui::Button("Remove All Animations"))
        {
            m_scene.clearAnimations();
            UpdateObjectsBuffer();
        }
        if (ImGui::Button("Reset All"))
        {
            m_scene.resetAnimations();
            UpdateObjectsBuffer();
        }
        if (ImGui::Button("Randomize Offsets"))
        {
            m_scene.randomizeAnimationOffsets();
            UpdateObjectsBuffer();
        }
        if (ImGui::Button("Randomize Animations"))
        {
            m_scene.randomizeAnimations();
            UpdateObjectsBuffer();
        }
    }
    ...
    if (!ImGui::CollapsingHeader("Individual Object Animation"))
    {
        ...
        ImGui::Combo("##xx", &currentAnimationSelection,
            ...
            (int)m_scene.m_meshes[m_scene.m_scene_objects[currentObjectSelection].mesh_id]->m_animationNamesCharP.size());
        ...
        ImGui::SameLine();
        if (ImGui::Button("Gen. Instances"))
        {
            Application::Get().Flush();
            //m_scene.init(m_model_file_path, 0);
            UnloadContent();
            m_scene.genInstances(currentObjectSelection, 5, 15);
            LoadContent();
            //UpdateObjectsBuffer();
        }
